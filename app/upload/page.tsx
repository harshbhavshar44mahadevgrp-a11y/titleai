...
const inputStyle: React.CSSProperties = {
    ...
    borderRadius: '8px', padding: '10px 13px', color: '#e2e8f0', fontSize: '12px',
    outline: 'none', fontFamily: 'Inter, system-ui, sans-serif', boxSizing: 'border-box',
}
const labelStyle: React.CSSProperties = {
    fontSize: '10px', color: '#475569', letterSpacing: '1.5px', fontWeight: '700',
    display: 'block', marginBottom: '6px',
}
const boundaryLabelStyle: React.CSSProperties = {
    fontSize: '10px', color: '#6366f1', letterSpacing: '1.5px', fontWeight: '700',
    display: 'block', marginBottom: '6px',
}
...
export default function UploadPage() {
    ...
    // Details sheet -- SAME AS ORIGINAL
    ...
    const addFiles = (newFiles: File[]) => {
        setFiles(prev => [...prev, ...newFiles.map(f => ({
            ...
            type: selectedType || 'Auto Detect', fileRef: f
        }))])
        setErrorMsg('')
        setReportData(null)
    }
    ...
    // ================================================================
    // ONLY CHANGE FROM ORIGINAL: PDF -> Images instead of text
    // Fixes Gujarati garbling issue in EC documents
    // ================================================================
    const convertPDFToImages = async (file: File): Promise<Array<{ data: string, mediaType: string }>> => {
        try {
            ...
            for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
                ...
                images.push({ data: base64, mediaType: 'image/png' })
            }
            return images
        } catch (e) {
        ...
        try {
            ...
            for (const f of files.filter(f => f.fileRef)) {
                ...
                if (file.type === 'application/pdf') {
                    // PDF -> Images (fixes Gujarati font garbling)
                    const pages = await convertPDFToImages(file)
                    allImages.push(...pages)
                } else if (file.type.startsWith('image/')) {
                    // Image file -> base64 directly
            ...
            const res = await fetch('/api/analyze', {
                ...
                body: JSON.stringify({
                    ...
                    applicantName: applicantNameInput,           // SAME
                    coApplicant: coApplicantInput || 'Not Applicable', // SAME
                    propertyAddress: propertyDescInput,          // SAME
                    currentOwner: currentOwnerInput,             // SAME
                    boundaryEast: boundaryEast || 'As per documents',   // SAME
                    boundaryWest: boundaryWest || 'As per documents',   // SAME
                    boundaryNorth: boundaryNorth || 'As per documents', // SAME
                    boundarySouth: boundarySouth || 'As per documents', // SAME
                })
            })
        ...
        <div style={{ minHeight: '100vh', background: '#020208', fontFamily: 'Inter, system-ui, sans-serif', display: 'flex', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, marginLeft: '225px', overflow: 'auto', position: 'relative', zIndex: 10 }}>
                ...
                <div style={{ padding: '32px' }}>
                    ...
                    {/* STEP 2 — DETAILS + UPLOAD (SAME AS ORIGINAL) */}
                    {!limitReached && caseSelected && !generating && !reportData && (
                        <>
                            ...
                            {/* CASE DETAILS SHEET -- SAME AS ORIGINAL */}
                    ...
                    {generating && (
                        <div style={{ background: 'rgba(2,2,8,0.9)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: '20px', padding: '48px 32px', textAlign: 'center' }}>
                            ...
                            <div style={{ fontSize: '12px', color: '#475569', marginBottom: '32px' }}>{selectedCase.icon} {selectedCase.label}</div>
