...
function sectionTitle(text: string) {
    return new Paragraph({
        ...
        children: [new TextRun({ text: '  ' + text, bold: true, size: 22, color: WHITE, font: 'Arial' })],
    ...
    try {
        ...
        const footer = new Footer({
            children: [new Paragraph({
                ...
                spacing: sp(60, 0),
                children: [new TextRun({ text: 'TitleAI & Associates  |  support@titleai.in  |  www.titleai.in  |  CONFIDENTIAL', size: 16, color: '888888', font: 'Arial' })],
            })],
        });
        ...
        s4.forEach((p: any) => {
            titleChainParas.push(new Paragraph({ spacing: sp(120, 60), children: [new TextRun({ text: `PARA ${p.paraNo} — ${p.heading || ''}`, bold: true, size: 20, color: DARK_BLUE, font: 'Arial' })] }));
            titleChainParas.push(new Paragraph({ spacing: sp(0, 120), children: [new TextRun({ text: p.content || '', size: 19, font: 'Arial', color: '222222' })] }));
        });
        ...
        riskParas.push(new Paragraph({ spacing: sp(80, 80), children: [new TextRun({ text: `OVERALL RISK: ${s5.overallRisk || 'LOW'}`, bold: true, size: 22, color: riskColor, font: 'Arial' })] }));
        ...
        if (s8.conditions?.length > 0) {
            conditionParas.push(new Paragraph({ spacing: sp(120, 60), children: [new TextRun({ text: '— SUBJECT TO THE FOLLOWING CONDITIONS —', bold: true, size: 20, color: opinionColor, font: 'Arial' })] }));
            s8.conditions.forEach((c: string) => {
                conditionParas.push(new Paragraph({ spacing: sp(0, 60), children: [new TextRun({ text: `—  ${c}`, size: 18, font: 'Arial' })] }));
            });
        }
            ...
            rows: [new TableRow({
                children: [
                    new TableCell({
                        ...
                        children: [para('Place: Ahmedabad / Gandhinagar'), para(`Date: ${meta.date || ''}`)],
                    }),
                    new TableCell({
                        borders,
                        margins: { top: 120, bottom: 120, left: 160, right: 160 },
        ...
        const doc = new Document({
            ...
            sections: [{
                ...
                children: [
                    ...
                    new Paragraph({ spacing: sp(60, 100), children: [new TextRun({ text: `EC STATUS: ${r.ecStatus || 'CLEAR'}`, bold: true, size: 20, color: (r.ecStatus || 'CLEAR') === 'CLEAR' ? '1E7A1E' : 'C00000', font: 'Arial' })] }),
                    emptyLine(),
                    ...
                    new Paragraph({ alignment: AlignmentType.CENTER, spacing: sp(80, 0), children: [new TextRun({ text: 'TitleAI & Associates  |  support@titleai.in  |  +91 98765 43210  |  www.titleai.in', size: 16, color: '888888', font: 'Arial' })] }),
                    new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `Ref: ${meta.refNo || ''}  |  Generated: ${meta.date || ''}  |  This report is computer generated and digitally authenticated by TitleAI & Associates.`, size: 14, color: 'AAAAAA', font: 'Arial', italics: true })] }),
                ],
            }],
        });
