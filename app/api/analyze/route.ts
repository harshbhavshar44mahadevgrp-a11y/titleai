...
?? BAJAJ HOUSING FINANCE LIMITED = a financial institution = BANK for this purpose
?? Any "HOUSING FINANCE" company = bank for release detection purposes
?? When BAJAJ HFL (or any bank) is in Col 3 ? they are RELEASING the mortgage ? RELEASE DEED
...
?? EC HEADER COUNT IS UNRELIABLE: "X registered transactions" in header = often WRONG.
...
------------------------------------------------
?? RELEASE DEED DETECTION — READ THIS CAREFULLY ??
------------------------------------------------
...
?? Read ALL EC_ROW_[N] and MORTGAGE_[N]_ANALYSIS from Layer 1.
?? MORTGAGE_STATUS=DISCHARGED = DO NOT flag as alert. Charge is satisfied.
...
?? DO NOT say "No discharge found" for any mortgage that Layer 1 marked as DISCHARGED.
?? A "???? ??..." entry in EC = the mortgage IS discharged. Do not say otherwise.
FINAL PARA:
...
7. ? RELEASE (TEXT): "Giro Muk" / "?í?o" + "??" / "?í?o ?u" = Release of Mortgage
...
?? IF MORTGAGE_STATUS=DISCHARGED in Layer 1 ? DO NOT flag as active encumbrance.
?? IF MORTGAGE_STATUS=ACTIVE ? flag in alerts as active mortgage.` }]
    })
