...
export function handleDownloadPDF(data: TitleReportData): void {
    ...
    const refNo = data.refNo || `TITLEMATRIX.AI/${new Date().getFullYear()}/DP`
    const reportDate = data.date || new Date().toLocaleDateString('en-IN')
    const advocateName = data.advocateName || 'TITLEMATRIX.AI Platform'
    ...
    // ── HEADER ──
    setFont('bold', 16)
    doc.text('TITLEMATRIX.AI', ML, y)
    setFont('normal', 9)
    doc.text('& ASSOCIATES', ML + 22, y)
    y += 6
    ...
    setFont('bold', 9)
    doc.text('Axis Bank Ltd.,', ML, y); y += 4
    doc.text(`${data.bankBranch || 'Ambawadi'},`, ML, y); y += 4
    ...
    setFont('normal', 8)
    doc.text('TITLEMATRIX.AI & Associates', PW - MR, y, { align: 'right' }); y += 4
    ...
    doc.text('support@TITLEMATRIX.AI.in', ML, 290)
    doc.text('www.TITLEMATRIX.AI.in', PW - MR, 290, { align: 'right' })
    ...
    setFont('normal', 8)
    const disclaimers = [
        'This report is generated by TITLEMATRIX.AI AI based on the documents provided.',
    ...
    doc.save(`TITLEMATRIX.AI_${name}_${reportDate.replace(/\//g, '-')}.pdf`)
}