...
export function handleDownloadPDF(data: TitleReportData): void {
    ...
    const refNo = data.refNo || `TitleAI/${new Date().getFullYear()}/DP`
    const reportDate = data.date || new Date().toLocaleDateString('en-IN')
    const advocateName = data.advocateName || 'TitleAI Platform'
    ...
    setFont('normal', 8)
    doc.text('TitleAI & Associates', PW - MR, y, { align: 'right' }); y += 4
    ...
    doc.text('support@titleai.in', ML, 290)
    doc.text('www.titleai.in', PW - MR, 290, { align: 'right' })
    ...
    setFont('normal', 8)
    const disclaimers = [
        'This report is generated by TitleAI AI based on the documents provided.',
    ...
    doc.save(`TitleAI_${name}_${reportDate.replace(/\//g, '-')}.pdf`)
}