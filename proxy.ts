...
// 2. Free users ko sirf FREE_LIMIT reports; uske baad 402 + /plans
// 3. Subscribed users (app_metadata.subscribed) unlimited
...
export async function proxy(req: NextRequest) {
    ...
    if (countRes.ok) {
        ...
        if (total >= FREE_LIMIT) {
            return deny(402, `Aapki ${FREE_LIMIT} free reports khatam ho gayi hain. Aage generate karne ke liye subscription lo.`, 'FREE_LIMIT_REACHED')
        }
    }
